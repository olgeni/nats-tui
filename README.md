...
- A value or a message body that spans several lines is piped to
  ...
  `nats kv update` reads no stdin, so an edited value is passed as an
  argument, quoted in the preview.
- `nats server check` exits non-zero on a warning or a critical state: that
  is the result, and the report is shown all the same.
- `B` tells a stream backup (a directory holding `backup.json`) from an
  account backup (one such directory per stream) and runs `nats stream
restore` or `nats account restore` accordingly.
